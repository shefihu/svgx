  ...
  const handlePaste = async () => {
    try {
      ...
      if (uploadedFiles.length + svgs.length > maxFiles) {
        alert(
          `Maximum ${maxFiles} files allowed. Found ${svgs.length} SVGs in clipboard.`
        );
        return;
      }
  ...
  return (
    <div className={className}>
      ...
      >
        ...
        <p className="text-sm text-white/50 mb-4">or click to browse files</p>
          ...
          <div className="mb-4 space-y-3">
            ...
            <div className="relative">
              ...
              <input
                ...
                className="w-full pl-10 pr-4 py-2 bg-white/5 border border-white/10 rounded-lg text-sm text-white placeholder:text-white/40 focus:outline-none focus:border-primary/50 transition-colors"
              />
            </div>
            ...
            <div className="flex items-center justify-between">
              <span className="text-sm text-white/60">
                {searchQuery
                  ? `${filteredFiles.length} of ${uploadedFiles.length}`
                    ...
                    <div className="flex-1 min-w-0">
                      {editingFileId === file.id ? (
                        <div className="relative">
                          ...
                          />
                          <div className="mt-1 text-[10px] text-primary flex items-center gap-2">
                        ...
                        <div
                          ...
                          onDoubleClick={() =>
                            file.status === 'success' && startEditing(file)
                          }
                        >
                          <p
                            className="text-sm text-white/90 truncate font-medium cursor-text select-none"
                            title={
                              file.status === 'success'
                          ...
                          {file.status === 'success' && (
                            <button
                              ...
                              className="opacity-0 group-hover:opacity-100 group-hover/name:opacity-100 p-1 hover:bg-primary/20 rounded transition-all"
                              title="Click to rename (or double-click filename)"
                            >
